...
#include "utils/memechain_bench_mark.h"
#include "ca/test.h"
...
int main(int argc, char* argv[])
{
	...
	struct sigaction sa;
	sa.sa_handler = sigHandlerOnther;
	...
	if(global::g_cpuNums < 1 || memory < 0.5 * 1024)
	{
		...
		std::cout << RED << "basic configuration:\n"
	...
	for (int i = 1; i < argc; i++)
	{
		...
		{
			showMenu = true;
			MagicSingleton<MEMECHAINBENCHMARK>::GetInstance()->OpenBenchmark();
		}
		else if (strcmp(argv[i], "-p") == 0)
		{
			showMenu = true;
			MagicSingleton<MEMECHAINBENCHMARK>::GetInstance()->OpenBenchmark2();
		}
		else
		{
			ERRORLOG("Parameter parsing error!");

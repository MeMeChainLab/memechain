#ifndef __memechain_wasmtime__
#define __memechain_wasmtime__
...
class WasmtimeVMhost {
  ...
  private:
    friend class MEMECHAINWasmtimeVM;
...
class MEMECHAINWasmtimeVM {
  public:
    MEMECHAINWasmtimeVM(WasmtimeVMhost *hostCaller, const std::string &bytes)
  ...
  private:
    wasmtime::_Result<std::string> _WasmtimeInit(int64_t gas, const std::string &wasmbytes,
        ...
        linker
            .func_wrap("env", "MEMECHAIN_result_size",
                       [&]() -> int32_t { return _result.size(); })
            .ok();

        linker
            .func_wrap("env", "MEMECHAIN_result",
                       [&](int32_t baseAddr) {
        ...
        linker
            .func_wrap("env", "MEMECHAIN_set_run_retvalue",
                       [&](int32_t baseAddr, int32_t lenght) {
                           std::string ret;
                           char p = cum->data(store)[baseAddr];
                           for (int i = 0; i < lenght; i++) {
        ...
        linker
            .func_wrap("env", "MEMECHAIN_input_param_size",
                       [&](int index) -> int32_t {
        ...
        linker
            .func_wrap("env", "MEMECHAIN_input_param",
...
template <typename RetType, typename... ParamsType> class MakeHost {
  public:
    MakeHost(MEMECHAIN::WasmtimeVMhost &Call, const std::string &name,
             const std::function<RetType(ParamsType...)> &bc) {
        MEMECHAIN::WASMTIMECaller<RetType, ParamsType...> *host =new MEMECHAIN::WASMTIMECaller<RetType, ParamsType...>(bc);

...
public:
    ...
    std::map<uint64_t, std::vector<std::string>> getHeightHashes() const;
...
private:
    ...
    std::map<uint64_t, std::vector<std::string>> heightHashes;

    // uint64_t computeHash(const std::string& input) const;
};
...
public:
    ...
    void broadcastVRF(const VRFConsensusInfo& vrf);
    void receiveVR(const VRFConsensusInfo& vrf);
    void run();
...
private:
    ...
    std::chrono::system_clock::time_point getNextCycleStart(std::chrono::system_clock::time_point current_time) const;
    std::chrono::system_clock::time_point getPreviousCycleTime(uint64_t timestamp) const;
    bool validateBlockinfo(const VRFConsensusInfo& vrfInfo, const std::string& nodeId) const;
